...
try {
    ...
    // $checkUsernameStmt = $pdo->prepare("SELECT id FROM admins_tbl WHERE username = ?");
    ...
    function generateAdminId($pdo) {
        ...
        $stmt = $pdo->query("SELECT COUNT(*) as count FROM admins_tbl");            $result = $stmt->fetch(PDO::FETCH_ASSOC);
...
// Also fix the config paths - replace the config section:
$possible_config_paths = [
    '../../config/database.php',    // This works! ✅
];
...
if (!$config_found) {
    logError("Database config not found. Checked paths: " . implode(', ', $possible_config_paths));
    throw new Exception("Database configuration file not found. Please check file structure.");
}
