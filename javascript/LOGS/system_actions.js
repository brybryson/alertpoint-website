...
function confirmExportLogs() {
    ...
    setTimeout(() => {
        ...
        let filename = 'system_action_logs_';
        ...
        if (!noResultsDiv) {
            ...
            const noResultsHtml = `
                <div id="noSearchResults" class="text-center py-8">
                    ...
                    <p class="text-gray-600">No system actions match your search</p>
                </div>
            `;
...
setInterval(function() {
    // Only refresh if we're not in a modal or actively searching
    const hasModals = document.querySelector('.fixed:not(.hidden)');
    const hasSearch = document.getElementById('searchInput').value.trim() !== '';
    
    if (!hasModals && !hasSearch) {
        location.reload();
    }
}, 30000);